...
def addPacketToList(socket, packet):
	list_of_outstanding_packets.append((socket, packet, 0));
...
def scan_for_timeouts(delay):
	...
	while ( True ):
		...
		time.sleep(delay)
		# example 
		for item in list_of_outstanding_packets:
			time_since = item[2];
			packet = item[1];
			socket = item[0];
			...
			if(packet.cntl == SYN):
				data = packet.pack();
				socket.udp_send(data);
				list_of_outstanding_packets.remove(item);
				dbg_print(1, "Transmitting: %s" % (packet.toHexFields()));
...
class Socket:
	...
	def __init__(self):
		self.udp = ip.socket(ip.AF_INET, ip.SOCK_DGRAM);
	...
	def set_drop_prob(self, probability):
		self.dropProbability = probability;
	...
	def bind(self,address):
		dbg_print(1, "CALL Bind: %s" % (str(address)));
	...
	def connect(self,address):
		dbg_print(1, "CALL Connect");
		
		self.address = address;
		# First, we ensure our state is correct
		if (self.state != STATE_INIT):
			dbg_print(1, "Invalid current state.");
			return;
		...
		pkt.cntl = SYN;
		pkt.seq = self.rand();
		addPacketToList(self, pkt);

		while (self.state == STATE_SYNSENT):
			received = self.udp_recv_packet();
	...
	def accept(self):
		...
		while (self.state == STATE_LISTEN):
			...
			send = Packet();
			send.cntl = SYN;
			send.ack = received.seq;
			send.seq = self.rand();
			addPacketToList(self, send);

		while (self.state == STATE_SYNRECV):
			a = 1 + 1;
	...
	def udp_recv_packet(self):
		recvdata = self.udp.recvfrom(MAX_PKT);
		data = recvdata[0];
		self.address = recvdata[1];
		pkt = Packet();
		pkt.unpack(data);
		return pkt;
